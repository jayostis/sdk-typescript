 ...
 *
 * TWO ENTRY POINTS, ONE READ. `runtimeImports(metafile)` is for a build in
 * progress, and is what `vendor-n3.mjs` and `check-browser-bundle.mjs` refuse
 * on. `runtimeImportsOf(code)` is for a file already on disk, and exists so
 * `tests/vendor-drift.test.ts` can ask the SHIPPED bundle the question
 * without trusting the builder's refusal — which has no test of its own, and
 * whose removal the byte-equality test cannot see.
 *
