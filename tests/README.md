...
| `tests/shacl/` | The SHACL evaluator, `src/shacl/evaluate.ts`, one file per concern — a component per pair, what goes unevaluated, targets, the report, and agreement with `rdf-validate-shacl`. `harness.ts` beside them holds both judges over one graph. | It is about how the engine JUDGES a graph against indexed shapes: hand-written Turtle for a single component, spec's shapes for agreement with the oracle. It is not about a rule a term declares (`tests/rules/`) nor about what `validate()` says of a record type (`tests/validator/`). |
