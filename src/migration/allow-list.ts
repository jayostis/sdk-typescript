 ...
 * three `imm-*` fixtures as graphs and finds no difference.
 *
 * It is routed for `serialize` ALONE, deliberately. There is no generic reader
 * yet — that is #78's `convertFromRdf` — and no generic validator — that is
 * #79's SHACL engine. Listing a path with nothing behind it would make the
...
export const MIGRATED_CLASSES: Readonly<Record<string, readonly MigrationPath[]>> = {
  'https://ns.cascadeprotocol.org/health/v1#ImmunizationRecord': ['serialize'],
};
